...
| Pico | 릴레이 모듈 |
|------|-----------|
| GP2 / 물리 4번핀 | IN1 / Signal |
| 5V   | VCC |
| GND  | GND |

- Pico 일반 버전에서는 RP2350 터치 대시보드에서 릴레이 카드를 터치해 ON/OFF 명령을 보낸다.
- Pico는 UART1 RX로 받은 명령에 따라 GP2 릴레이 출력을 제어한다.
- 현재 테스트한 4채널 릴레이 모듈은 active-low 타입이다.
  - 논리 OFF: `relay cmd 0`, GP2 HIGH, 릴레이 OFF
  - 논리 ON: `relay cmd 1`, GP2 LOW, 릴레이 ON
