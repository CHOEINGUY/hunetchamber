...
def load_config():
    """Load persistent config; fallback to defaults."""
    try:
...
def save_config(cfg):
    ...
    try:
        ...
        with open(tmp, "w") as f:
            json.dump(cfg, f)
        uos.rename(tmp, CONFIG_FILE)
    except OSError as e:
        print("Failed to save config:", e)


# Load persistent settings
_persistent = load_config()
...
armed = False
auto_mode = False
target_c = _persistent["target_c"]
band_high_c = _persistent["band_high_c"]
band_low_c = _persistent["band_low_c"]
fan_percent = 0
led_percent = 0
...
def read_temp_humidity(addr):
    req = make_read(addr, 0x0000, 2)
    while uart.any():
        uart.read()
    uart.write(req)
    time.sleep_ms(250)
    raw = uart.read() if uart.any() else None
    if not raw:
        return None
    # strip TX echo if transceiver loops back transmitted bytes
...
def apply_auto_control():
    ...
    if not auto_mode:
        return
    if current_temp_c is None or sensor_age_seconds(sensors[0]) > SENSOR_STALE_S:
        last_control_reason = "no_temp"
        return

    upper = target_c + band_high_c
    lower = target_c - band_low_c
...
def status_line():
    return (
        "STATUS on={} armed={} auto={} target_c={} band_c={} band_low_c={} "
        ...
        "temp_c={} humidity={} sensor_age_s={} fan={} led={} reason={}".format(
            ...
            value_or_na(band_high_c),
            value_or_na(band_low_c),
            min_off_ms // 1000,
...
def fridge_payload():
    stale = sensor_age_seconds(sensors[0]) > SENSOR_STALE_S
    return {
        "device_id": DEVICE_ID,
        "temp_c": None if stale else json_value(current_temp_c),
        "humidity": None if stale else json_value(current_humidity),
        "target_c": target_c,
        "band_c": band_high_c,
        "band_low_c": band_low_c,
...
def post_gateway_payload(payload):
    ...
    try:
        ...
        resp = b""
        deadline = time.ticks_add(time.ticks_ms(), HTTP_TIMEOUT_SECONDS * 1000)
        ...
        if sep >= 0 and len(resp) > sep + 4:
            try:
                data = json.loads(resp[sep + 4:].decode())
                cmd = data.get("cmd")
                if cmd:
                    print("GATEWAY CMD {}".format(cmd))
                    handle_command(str(cmd))
            except Exception as e:
                print("GATEWAY CMD PARSE ERROR {}".format(e))
        return True
    finally:
        s.close()
...
def handle_command(line):
    global armed, auto_mode, target_c, band_high_c, band_low_c, min_off_ms, min_on_ms, _persistent
    ...
    if cmd == "help":
        print("COMMANDS: status arm disarm on off forceoff auto <0|1> target <0..25> bandhigh <0..5> bandlow <0..5> fan <0|100> led <0..100> minoff <seconds> minon <seconds>")
    elif cmd == "status":
        print_status()
    elif cmd == "arm":
        armed = True
        ...
        _persistent["target_c"] = target_c
        save_config(_persistent)
        print("OK target_c={}".format(value_or_na(target_c)))
    elif cmd == "bandhigh":
        if len(parts) != 2:
            print("ERR usage: bandhigh <0..5>")
            return
        value = parse_float(parts[1])
        if value is None:
            print("ERR bandhigh must be a number")
            return
        band_high_c = round(clamp(value, 0.1, 5.0), 1)
        _persistent["band_high_c"] = band_high_c
        save_config(_persistent)
        print("OK band_high_c={}".format(value_or_na(band_high_c)))
    elif cmd == "bandlow":
        if len(parts) != 2:
            print("ERR usage: bandlow <0..5>")
            return
        value = parse_float(parts[1])
        if value is None:
            print("ERR bandlow must be a number")
            return
        band_low_c = round(clamp(value, 0.1, 5.0), 1)
        _persistent["band_low_c"] = band_low_c
        save_config(_persistent)
        print("OK band_low_c={}".format(value_or_na(band_low_c)))
    elif cmd == "fan":
        if len(parts) != 2:
