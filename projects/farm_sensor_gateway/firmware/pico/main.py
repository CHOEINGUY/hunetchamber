...
# ── 핀 설정 ──────────────────────────────────────────
led   = Pin(25, Pin.OUT)
relay_fan = Pin(2, Pin.OUT, value=1)   # active-low relay: 1=OFF, 0=ON
relay_pump1 = Pin(3, Pin.OUT, value=1) # active-low relay: 1=OFF, 0=ON
relay_pump2 = Pin(7, Pin.OUT, value=1) # active-low relay: 1=OFF, 0=ON
relay_led = Pin(8, Pin.OUT, value=1)   # active-low relay: 1=OFF, 0=ON
relay_mask = 0
...
def set_relay(cmd):
    global relay_mask
    relay_mask = cmd & 0x0F
    relay_fan.value(0 if (relay_mask & 0x01) else 1)
    relay_pump1.value(0 if (relay_mask & 0x02) else 1)
    relay_pump2.value(0 if (relay_mask & 0x04) else 1)
    relay_led.value(0 if (relay_mask & 0x08) else 1)
