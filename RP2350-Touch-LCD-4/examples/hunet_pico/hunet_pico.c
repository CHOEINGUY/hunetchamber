...
static void uart_rx_irq_handler(void) {
    while (uart_is_readable(PICO_UART)) {
        ...
        if (c == '?') {
            uart_putc_raw(PICO_UART, g_relay_cmd[0]); 
            continue;
        }
...
static void create_dashboard(void) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        lv_obj_t *rlbl = lv_label_create(relay_cards[i]);
        char rname[16]; snprintf(rname, sizeof(rname), "RELAY %d", i + 1);
        lv_label_set_text(rlbl, rname);
        lv_obj_set_style_text_font(rlbl, &lv_font_montserrat_12, 0);
        lv_obj_set_style_text_color(rlbl, lv_color_hex(is_dark_mode ? 0x94a3b8 : 0x64748b), 0);
