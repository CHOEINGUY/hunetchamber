...
// ── I2C slave (Pico → RP2350) ────────────────────────
// I2C1: SDA=GP6, SCL=GP7  (보드의 SDA/SCL 커넥터)
// bsp_i2c_init() / lv_port_indev_init() 호출 안 함 (터치 비활성화)
...
static lv_obj_t *val_ec,       *val_ph,        *val_relay;
static lv_obj_t *val_n,        *val_p,         *val_k;
static lv_obj_t *lbl_status;

// ── I2C1 슬레이브 IRQ ─────────────────────────────────
static void i2c_slave_irq_handler(void) {
    uint32_t stat = i2c_get_hw(i2c1)->intr_stat;

    if (stat & I2C_IC_INTR_STAT_R_RX_FULL_BITS) {
        char c = (char)(i2c_get_hw(i2c1)->data_cmd & 0xFF);
    ...
    if (stat & I2C_IC_INTR_STAT_R_STOP_DET_BITS) {
        (void)i2c_get_hw(i2c1)->clr_stop_det;
    }
}
...
static bool parse_csv(const char *s, SensorData *d) {
    ...
    d->co2       = (int)v[10]; d->relay = (int)v[11];
    return true;
}
...
static void create_dashboard(void) {
    ...
    // 카드 크기: 3열 / 4행
    int cw = (480 - 8*2 - 8*2) / 3;   // 약 144px
    int ch = (436 - 8*2 - 8*3) / 4;   // 약 98px
    ...
    c = make_card(grid, "Relay",     0x64748b, &val_relay);      lv_obj_set_size(c, cw, ch);
    // 행 3: NPK
...
static void update_labels(const SensorData *d) {
    ...
    lv_obj_set_style_text_color(val_relay,
        lv_color_hex(d->relay ? 0x10b981 : 0x64748b), 0);
...
int main(void) {
    ...
    set_cpu_clock(260);
    i2c_slave_setup();  // I2C1 GP6/GP7 — Pico 수신 (터치 비활성화)
